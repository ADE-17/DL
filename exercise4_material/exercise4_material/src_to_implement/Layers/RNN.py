...
class RNN(Base.BaseLayer):
    ...
    def forward(self, input_tensor):
        ...
        for t in range(self.time_steps):
            hidden_st_temp = self.hidden_state[t][np.newaxis, :] # was "hiddenstates", changed to hidden_st_temp
            input = input_tensor[t][np.newaxis, :]
            
            # x = input_tensor[t].reshape(1,-1) # No need to reshape now
            xs = np.concatenate((hidden_st_temp, input), axis = 1)
